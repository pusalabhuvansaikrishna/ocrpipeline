...
export default function PrintedOCR() {
  ...
  const [showLanguageWarning, setShowLanguageWarning] = useState(false); // ← new
  ...
  const handleDownloadAll = async () => {
    ...
    try {
      ...
      const a = document.createElement("a");
      a.href = url;
      a.download = `FAST_OCR_Printed_${new Date().toISOString().slice(0, 10)}.zip`;
      a.click();
      URL.revokeObjectURL(url);
    } catch (err) {
      console.error(err);
      ...
      <div className="max-w-7xl mx-auto px-5 sm:px-6 lg:px-8 py-10 md:py-16">
        ...
        <div className="text-center mb-12">
          ...
          <p className="mt-3 text-lg text-gray-400 max-w-2xl mx-auto">
            Extract clean text from scanned documents, books, screenshots — fast & accurate
          </p>
        </div>
        ...
        <div className="flex flex-col sm:flex-row justify-center items-center gap-6 mb-10 relative">
          <div className="w-full sm:w-auto">
            <label className="block text-sm text-gray-400 mb-1.5">Recognition Language <span className="text-red-400">*</span></label>
            <select
