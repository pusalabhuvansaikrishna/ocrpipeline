...
const getStatusStyle = (status: string): React.CSSProperties => {
  switch (status.toLowerCase()) {
    case "queued":
      return { backgroundColor: "rgba(234,179,8,0.2)", color: "#fde047" };
    case "processing":
      return { backgroundColor: "rgba(59,130,246,0.2)", color: "#93c5fd" };
    case "completed":
...
// Computes a status badge style based on actual progress outcomes
const getJobBadgeStyle = (job: Job): React.CSSProperties => {
  const { completed, failed, total } = job.progress;
  ...
  if (status === "completed") {
    ...
    if (failed > 0) {
      // Partially failed
      return { backgroundColor: "rgba(249,115,22,0.2)", color: "#fdba74" };
    }
    // All succeeded
    return { backgroundColor: "rgba(34,197,94,0.2)", color: "#86efac" };
  }
...
// Progress bar color: green when done, blue when processing, orange for queued, never red
const getProgressBarColor = (job: Job): string => {
  const status = job.status.toLowerCase();
  if (status === "completed") return "#4ade80";
  if (status === "processing") return "#60a5fa";
  return "#fb923c"; // orange for queued / anything else
};
                  ...
                  <div style={{ padding:"14px 18px", borderBottom:"1px solid #1e293b", display:"flex", alignItems:"center", justifyContent:"space-between" }}>
                    ...
                    <div style={{ display:"flex", gap:8 }}>
                      {Object.entries(
                        ...
                        // Find a matching job to derive style
