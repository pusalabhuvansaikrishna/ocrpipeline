...
export default function SceneOCR() {
  ...
  const handleProcess = useCallback(async () => {
    ...
    try {
      ...
      const res = await fetch(`${API_BASE_URL}/api/ocr/scene`, {
        method: "POST",
        body: formData,
      });
  ...
  const handleDownloadAll = async () => {
    ...
    try {
      ...
      const a = document.createElement("a");
      a.href = url;
      a.download = `FAST_OCR_Scene_${new Date().toISOString().slice(0, 10)}.zip`;
      a.click();
      URL.revokeObjectURL(url);
    } catch (err) {
      console.error(err);
      ...
      <div className="max-w-7xl mx-auto px-5 sm:px-6 lg:px-8 py-10 md:py-16">
        ...
        <div className="text-center mb-12">
          ...
          <p className="mt-3 text-lg text-gray-400 max-w-2xl mx-auto">
            Extract text from photos, signs, posters, whiteboards, handwriting — real-world scenes
          </p>
        </div>
