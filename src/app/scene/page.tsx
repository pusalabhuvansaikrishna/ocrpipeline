...
export default function PrintedOCR() {
  ...
  const handleProcess = useCallback(async () => {
    ...
    try {
      ...
      const response = await fetch(`${API_BASE_URL}/api/ocr/scene`, {
        method: "POST",
        body: formData,
      });
      ...
      setSuccessMessage(`Processed ${data.count || selectedFiles.length} scene document(s) successfully`);
  ...
  const handleDownloadAll = async () => {
    ...
    try {
      ...
      const link = document.createElement("a");
      link.href = url;
      link.download = `FASTOCR_Scene_Results_${new Date().toISOString().slice(0, 10)}.zip`;
      link.click();
      URL.revokeObjectURL(url);
    } catch (err) {
          ...
          <div className="mt-10 flex justify-center">
            ...
            >
              {isProcessing ? (
                ...
                "Extract Scene Text from All Files"
              )}
            </button>
          </div>
